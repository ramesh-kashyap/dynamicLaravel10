@foreach ($formData as $data)
    <div class="form-group">
        ...
        @elseif($data->type == 'file')
            <input type="file" class="form-control custom-input-field" name="{{ $data->label }}" @if ($data->is_required == 'required') required @endif accept="@foreach (explode(',', $data->extensions) as $ext) .{{ $ext }}, @endforeach">
